...
function inspect(files, outputDir, options, done) {	
	...
	filtered = files.filter(function (str) { return str.indexOf(PATTERN) === -1; });
	filtered = filtered.filter(function (str) { return str.indexOf('/config/') === -1; });
	filtered = filtered.filter(function (str) { return str.indexOf('/www/') === -1; });
	filtered = filtered.filter(function (str) { return str.indexOf('-') === -1; });
	filtered = filtered.filter(function (str) { return str.indexOf('.png') === -1; });
	filtered = filtered.filter(function (str) { return str.indexOf('.jpeg') === -1; });
	filtered = filtered.filter(function (str) { return str.indexOf('.txt') === -1; });
	filtered = filtered.filter(function (str) { return str.indexOf('.d.') === -1; });
	filtered = filtered.filter(function (str) { return str.indexOf('.html') === -1; });
	filtered = filtered.filter(function (str) { return str.indexOf('.scss') === -1; });
	filtered = filtered.filter(function (str) { return str.indexOf('.css') === -1; });
	filtered = filtered.filter(function (str) { return str.indexOf('.ico') === -1; });
	filtered = filtered.filter(function (str) { return str.indexOf('.xml') === -1; });
	
	files=filtered;
	//   var PATTERN = ['node_modules','test','-config'];	  
    // var filtered = files.filter(value => !PATTERN.includes(value));;
	// files=filtered;
	...
	function runReports(files, done) {		
		files.forEach(function(file) {			
			...
			if (options.recurse && fs.statSync(file).isDirectory()) {				
				...
				runReports(files);
			} else if (file.match(/\.jsx?$/) || file.match(/\.ts$/)) {
