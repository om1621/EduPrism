  ...
  // Project configuration.
	grunt.initConfig({
		...
		uglify: {
			'assets': {
				files: {
					...
					'lib/assets/scripts/bundles/codemirror.js': ['lib/assets/scripts/vendor/codemirror/codemirror.js', 'lib/assets/scripts/vendor/codemirror/javascript.js', 'lib/assets/scripts/vendor/codemirror/util/searchcursor.js']
				}
			}
		},
		watch: {
		...
		casper: {
			test: {
				files: {
					'reports/casper.xml': ['test/casper-overview.js', 'test/casper-sortable-file-list.js']
				},
				options: {
					test: true,
					verbose: true,
	...
	grunt.registerTask('runtest', function() {
		...
		grunt.util.spawn({
			...
			if (err || code !== 0) {
				grunt.fatal('Running jschecker binary failed');
			}
			done();
		});
	});
